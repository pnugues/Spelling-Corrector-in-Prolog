...
edits1(WAtom, EdtAtoms) :-
	atom_chars(WAtom, W),
	findall(Del, (append(Start, [_|End], W), append(Start, End, Del)), Dels),
	findall(Transp, (append(Start, [X, Y|End], W), append(Start, [Y, X|End], Transp)), Transps),
	findall(Repl, (append(Start, [_|End], W), letter(L), append(Start, [L|End], Repl)), Repls),
	findall(Insert, (append(Start, End, W), letter(L), append(Start, [L|End], Insert)), Inserts),
	setof(X, (member(X, Dels); member(X, Transps); member(X, Repls);
	member(X, Inserts)), E),
	maplist(atom_chars, EdtAtoms, E), !.

known(Words, Dict, Known) :-
	findall(W, (member(W, Words), _ = Dict.get(W)), Known).

known_edits2(Word, Dict, KnownUniqEds2) :-
	edits1(Word, Ed1s),
	findall(Ed2,
		(member(Ed1, Ed1s), edits1(Ed1, Eds2), member(Ed2, Eds2), _ = Dict.get(Ed2)),
		KEds2), !,
...
count_occurrences(Text, Occs):-
    findall(W:Cnt, (bagof(true, member(W, Text), Ws), length(Ws,Cnt)), Occs).

test1(['access': 'acess', 'accessing': 'accesing', 'accommodation':
...
test2(['forbidden': 'forbiden', 'decisions': 'deciscions descisions',
